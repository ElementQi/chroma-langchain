...
class QwenAIClient:
    def __init__(self, model_name, api_key=None):
        ...
        self.client = OpenAI(
            api_key=api_key, base_url="https://one-api.s.metames.cn:38443/v1"
        )
        self.model_name = model_name
...
class LLMSemanticSplitter(TextSplitter):
    """
    LLMSemanticSplitter is a class designed to split text into thematically consistent sections based on suggestions from a Language Model (LLM).
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if organisation == "openai":
            ...
            raise ValueError(
                "Invalid organisation. Please choose either 'openai' or 'anthropic' or 'qwen'."
            )
